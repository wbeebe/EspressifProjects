 ...
 *          i2c address
 */
Adafruit_BNO055::Adafruit_BNO055(uint8_t address) {
    device_address = address;
}

esp_err_t Adafruit_BNO055::initialize() {
    return ESP_OK;
}

esp_err_t Adafruit_BNO055::reset() {
    return ESP_OK;
}

esp_err_t Adafruit_BNO055::test() {
    return ESP_OK;
}
...
bool Adafruit_BNO055::begin(adafruit_bno055_opmode_t mode) {
  //
  // This was rewritten from the original Adafruit code to check for the
  // ESP-IDF I2C return type on the action as well as the device ID.
  // This is more robust and the calls to the underlying ESP-IDF I2C libraries
  // are more efficient. The original code would lock into an endless loop
  // otherwise.
  //
  // This is how I will try to do this in the rest of the BNO055 code.
  //
  if (initialize_device_handle() != ESP_OK) {
  ...
  auto error = i2c_master_transmit_receive(device_handle,
    data, sizeof(data), data, 1, I2C_TICKS_TO_WAIT);

  if (error != ESP_OK or data[0] != BNO055_ID) {
    ESP_LOGI(TAG, "CHIP IDENTIFICATION FAILED");
    return false;
  }
  ...
  if (i2c_master_transmit(device_handle, data, 2, I2C_TICKS_TO_WAIT) != ESP_OK) {
    ESP_LOGI(TAG, "FAILED TO SOFTWARE RESET CHIP");
    return false;
  }

  // Delay increased to 1000ms/1s after explicit checking inside a do/while loop
  // checking to see how long the BNO055 came out of its explicit reset.
  //
  // Remove the do/while loop and only do this once. If you can't do it this
  // one time, then you've failed.
  //
  vTaskDelay(1000 / portTICK_PERIOD_MS);

  data[0] = BNO055_CHIP_ID_ADDR;
  error = i2c_master_transmit_receive(device_handle, data, 1, data, 1,
                                     I2C_TICKS_TO_WAIT);
  ESP_LOGI(TAG,"CHECK TO SEE IF BNO055 COMES OUT OF SOFTWARE RESET");

  if (error != ESP_OK or data[0] != BNO055_ID) {
    ESP_LOGI(TAG,"FAILED TO COME OUT OF SOFTWARE RESET");
    return false;
  }
...
int8_t Adafruit_BNO055::getTemp() {
  return static_cast<int8_t>(read8(BNO055_TEMP_ADDR));
}
...
imu::Vector<3> Adafruit_BNO055::getVector(adafruit_vector_type_t vector_type) {
  ...
  x = static_cast<uint16_t>(buffer[0]) | (static_cast<uint16_t>(buffer[1]) << 8);
  y = static_cast<uint16_t>(buffer[2]) | (static_cast<uint16_t>(buffer[3]) << 8);
  z = static_cast<uint16_t>(buffer[4]) | (static_cast<uint16_t>(buffer[5]) << 8);
  ...
  case VECTOR_EULER:        // 1 degree = 16 LSB
    xyz[0] = static_cast<double>(x) / 16.0;
    xyz[1] = static_cast<double>(y) / 16.0;
    xyz[2] = static_cast<double>(z) / 16.0;
    break;
  case VECTOR_ACCELEROMETER:  // 1m/s^2 = 100 LSB
  case VECTOR_LINEARACCEL:    // 1m/s^2 = 100 LSB
  case VECTOR_GRAVITY:        // 1m/s^2 = 100 LSB
    xyz[0] = static_cast<double>(x) / 100.0;
    xyz[1] = static_cast<double>(y) / 100.0;
    xyz[2] = static_cast<double>(z) / 100.0;
    break;
  }
...
imu::Quaternion Adafruit_BNO055::getQuat() {
  ...
  x = (static_cast<uint16_t>(buffer[3]) << 8) | static_cast<uint16_t>(buffer[2]);
  y = (static_cast<uint16_t>(buffer[5]) << 8) | static_cast<uint16_t>(buffer[4]);
  z = (static_cast<uint16_t>(buffer[7]) << 8) | static_cast<uint16_t>(buffer[6]);
...
bool Adafruit_BNO055::write8(adafruit_bno055_reg_t reg, uint8_t value) {
  ...
  auto error = i2c_master_transmit(
    device_handle, buffer, sizeof(buffer), I2C_TICKS_TO_WAIT);
  return error == ESP_OK;
}
...
uint8_t Adafruit_BNO055::read8(adafruit_bno055_reg_t reg) {
  ...
  i2c_master_transmit_receive(device_handle,buffer,1,buffer,1,
...
bool Adafruit_BNO055::readLen(adafruit_bno055_reg_t reg, uint8_t *buffer,
  ...
  auto error = i2c_master_transmit_receive(
    device_handle, (uint8_t *)&reg, 1, reg_buf, sizeof(reg_buf), I2C_TICKS_TO_WAIT);
  return error == ESP_OK;
}
